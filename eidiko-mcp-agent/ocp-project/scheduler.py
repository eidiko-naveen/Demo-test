...
# Allow `python3 scheduler.py` from the project root without requiring users to
# activate the virtual environment first. Dependencies remain isolated in
# .venv; this process is replaced before importing any third-party packages.
...
def _write_heartbeat() -> None:
    """Atomically update the scheduler heartbeat consumed by container probes."""
    path = Path(cfg.scheduler_health_file)
    try:
        ...
        log.error("scheduler_heartbeat_failed", path=str(path), error=str(exc))
...
def _run_remediation_job() -> None:
    """Process one human-approved request; a disabled execution gate is a no-op."""
    from agent.remediation import execute_one_approved_request

    try:
        execute_one_approved_request()
    except Exception as exc:
        log.error("remediation_job_failed", error=str(exc), exc_info=True)
...
def _startup_checks() -> None:
    ...
    # This monitor queries OpenShift-specific API groups. Refuse plain
    # Kubernetes targets (including Minikube) to prevent false alerts.
    if cfg.require_openshift:
        log.info("startup_check", check="openshift_target")
        try:
            ...
            log.error(
                ...
                check="openshift_target",
                reason="Configured kubeconfig is unreachable or is not OpenShift.",
                error=str(exc),
            )
            sys.exit(1)
        log.info("startup_check_passed", check="openshift_target")
...
def main() -> None:
    ...
    scheduler.add_job(
        ...
        trigger=IntervalTrigger(seconds=cfg.hitl_poll_seconds, timezone=cfg.timezone),
        id="hitl_remediation",
        name="Human-approved remediation executor",
