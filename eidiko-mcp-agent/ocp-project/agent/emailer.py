...
def dispatch(subject: str, html_body: str, recipients: Optional[List[str]] = None) -> None:
    """
    ...
    When recipients is omitted, the configured monitoring recipients are used.
    A caller may pass a narrow recipient list for approval notifications.
    """
    recipients = recipients if recipients is not None else cfg.email_recipients
    if not recipients:
