...
# Support `python -m agent.dashboard` without requiring manual activation.
# Relaunch inside the project virtual environment before third-party imports.
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
_VENV_PYTHON = _PROJECT_ROOT / ".venv" / "bin" / "python"
if __name__ == "__main__" and sys.prefix == sys.base_prefix and _VENV_PYTHON.exists():
    os.execv(
        str(_VENV_PYTHON),
        [str(_VENV_PYTHON), "-m", "agent.dashboard", *sys.argv[1:]],
    )
