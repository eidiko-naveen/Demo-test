...
def _section(title: str, content: str, icon: str = "") -> str:
    return f"""
<div style="margin:0 0 20px 0;padding:20px;background:#FFFFFF;border:1px solid {_C['border']};border-radius:12px;">
...
def _table(headers: List[str], rows: List[List[str]], col_widths: Optional[List[str]] = None) -> str:
    th_style = (
        ...
        f"padding:10px 12px;text-align:left;{_FONT}font-size:11px;letter-spacing:.04em;text-transform:uppercase;"
    )
    td_style = f"padding:10px 12px;border-bottom:1px solid {_C['border']};{_FONT}font-size:12px;line-height:1.45;"
    ...
    return f"""
<table role="presentation" style="width:100%;border-collapse:separate;border-spacing:0;border:1px solid {_C['border']};border-radius:8px;overflow:hidden;margin-bottom:8px;">
...
def _empty_notice(message: str = "No issues detected.") -> str:
    return (
        ...
        f'padding:12px 14px;background:{_C["green_bg"]};border:1px solid #A7F3D0;'
        f'border-radius:8px;">✔ {_e(message)}</p>'
    )
...
def _code_block(commands: List[str]) -> str:
    ...
    return (
        ...
        f'border-radius:8px;font-size:12px;overflow-x:auto;border-left:4px solid #38BDF8;'
...
def _build_header(state: ClusterState) -> str:
    ...
    return f"""
<div style="background:{_C['header_bg']};padding:30px 32px;border-radius:14px 14px 0 0;margin:0;">
  <table role="presentation" style="width:100%;border-collapse:collapse;">
    <tr>
      <td style="vertical-align:top;">
        <div style="color:#7DD3FC;font-size:11px;font-weight:bold;letter-spacing:1.5px;text-transform:uppercase;{_FONT}">
          OCP AI OPERATIONS
        </div>
        <div style="color:{_C['white']};font-size:26px;font-weight:bold;line-height:1.2;margin-top:7px;{_FONT}">
          Cluster Health Briefing
        </div>
        <div style="color:#BFDBFE;font-size:13px;margin-top:7px;{_FONT}">
          {cluster} &nbsp;&bull;&nbsp; {timestamp}
        </div>
      ...
      <td style="text-align:right;vertical-align:top;">
        <div style="background:{status_bg};color:{status_color};
                    ...
                    border-radius:24px;{_FONT}display:inline-block;border:1px solid rgba(255,255,255,.25);">
          {status_label}
        </div>
...
<div style="background:#EFF6FF;padding:18px 28px;border-left:4px solid #38BDF8;
            margin:0 0 20px 0;border-radius:0 0 12px 12px;">
...
def _build_overview(state: ClusterState) -> str:
    """Gmail-safe executive summary tiles using presentation tables."""
    nodes = [n for n in state.get("nodes", []) if "error" not in n]
    operators = [o for o in state.get("operators", []) if "error" not in o]
    failures = state.get("failures", [])
    errors = state.get("collection_errors", {})
    ready_nodes = sum(1 for n in nodes if n.get("ready"))
    healthy_ops = sum(
        1 for o in operators
        if o.get("available") and not o.get("degraded")
    )

    tiles = [
        (f"{ready_nodes}/{len(nodes)}", "Ready nodes", _C["green"], _C["green_bg"]),
        (f"{healthy_ops}/{len(operators)}", "Healthy operators", _C["blue"], _C["blue_bg"]),
        (str(len(failures)), "Detected issues", _C["red"] if failures else _C["green"], _C["red_bg"] if failures else _C["green_bg"]),
        (str(len(errors)), "Collection errors", _C["amber"] if errors else _C["purple"], _C["amber_bg"] if errors else _C["purple_bg"]),
    ]
    cells = "".join(
        f'<td width="25%" style="padding:0 5px;vertical-align:top;">'
        f'<div style="background:{bg};border:1px solid {_C["border"]};border-radius:10px;padding:15px 12px;text-align:center;">'
        f'<div style="{_FONT}font-size:23px;font-weight:bold;color:{color};">{_e(value)}</div>'
        f'<div style="{_FONT}font-size:10px;color:{_C["subtext"]};text-transform:uppercase;letter-spacing:.5px;margin-top:5px;">{_e(label)}</div>'
        f'</div></td>'
        for value, label, color, bg in tiles
    )
    return f'<table role="presentation" style="width:100%;border-collapse:collapse;margin:0 0 20px 0;"><tr>{cells}</tr></table>'
...
def build_html_report(state: ClusterState) -> str:
    ...
    body = (
        ...
        + _safe(lambda: _build_overview(state),           "Overview")
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    body {{ {_BASE} background:#EEF2FF; }}
    a {{ color:{_C['blue']}; }}
  </style>
</head>
<body>
  <div style="display:none;max-height:0;overflow:hidden;color:#EEF2FF;">OpenShift cluster health, issues, and remediation summary.</div>
  <div style="max-width:920px;margin:20px auto;background:#F8FAFC;
              border-radius:14px;box-shadow:0 8px 32px rgba(15,23,42,0.14);overflow:hidden;border:1px solid #DCE4F2;">
