...
def _get_vector_store():
    ...
    async_connection_string = URL.create(
    ...
    store = PGVectorStore.from_params(
        # ``str(URL)`` masks passwords as ``***``. PGVectorStore coerces URL
        # objects with str(), so render explicitly or authentication fails.
        connection_string=connection_string.render_as_string(hide_password=False),
        async_connection_string=async_connection_string.render_as_string(
            hide_password=False
        ),
