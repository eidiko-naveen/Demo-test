...
Two chains using Groq-hosted language models:
...
def _get_llm() -> ChatGroq:
    """Return the configured Groq-backed LangChain chat model."""
    return ChatGroq(
        groq_api_key=cfg.groq_api_key,
        model=cfg.llm_model,
        temperature=cfg.llm_temperature,
        max_tokens=cfg.llm_max_tokens,
        max_retries=0,  # Tenacity below owns retry timing and logging.
    )
...
def _trim_snapshot(state: ClusterState) -> Dict[str, Any]:
    ...
    # ── Nodes ────────────────────────────────────────────────────────────────
    all_nodes = [n for n in state.get("nodes", []) if "error" not in n]
    unhealthy_nodes = []
    for n in all_nodes:
    ...
    # ── Operators ────────────────────────────────────────────────────────────
    all_operators = [op for op in state.get("operators", []) if "error" not in op]
    unhealthy_operators = []
    ...
    # ── Pods ─────────────────────────────────────────────────────────────────
    all_pods = [pod for pod in state.get("pods", []) if "error" not in pod]
    unhealthy_pods = []
    for pod in all_pods:
    ...
    # ── PVCs ─────────────────────────────────────────────────────────────────
    all_pvcs = [p for p in state.get("pvcs", []) if "error" not in p]
    unhealthy_pvcs = [p for p in all_pvcs if _is_pvc_unhealthy(p)]
    healthy_pvc_count = len(all_pvcs) - len(unhealthy_pvcs)

    # ── MachineConfigPools ────────────────────────────────────────────────────
    all_mcpools = [m for m in state.get("mcpools", []) if "error" not in m]
    ...
    # ── Certificates ─────────────────────────────────────────────────────────
    all_certs = [c for c in state.get("certs", []) if "error" not in c]
    unhealthy_certs = [c for c in all_certs if _is_cert_unhealthy(c)]
    healthy_cert_count = len(all_certs) - len(unhealthy_certs)

    # ── CP4I Endpoints ────────────────────────────────────────────────────────
    all_endpoints = [e for e in state.get("cp4i_endpoints", []) if "error" not in e]
...
- Telemetry/API collection errors are not cluster-resource failures and must never
  be included in the failures list. They are handled separately by the application.
