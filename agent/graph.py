...
def get_llm_model(tools: List[Any], requested_model: str = None):
    req_model = requested_model or LLM_MODEL
    req = req_model.lower()
    is_groq_req = any(req.startswith(prefix) for prefix in ["llama", "qwen", "groq", "openai/"])

    if is_groq_req and GROQ_API_KEY:
        try:
            ordered = list(dict.fromkeys([req_model] + GROQ_MODEL_CANDIDATES))
            bound = _build_groq_llm(ordered[0], tools)
            print(f"[LLM] Initialized with Main Groq model: {ordered[0]}")
            return bound
        except Exception as e:
            print(f"[LLM] Could not build Groq LLM ({req_model}): {e}")

    if ANTHROPIC_API_KEY and ANTHROPIC_API_KEY not in ("", "your_anthropic_api_key_here"):
        try:
            start_model = requested_model if (requested_model and not is_groq_req) else FALLBACK_LLM_MODEL
            ordered = list(dict.fromkeys([start_model] + ANTHROPIC_MODEL_CANDIDATES))
            bound = _build_anthropic_llm(ordered[0], tools)
            print(f"[LLM] Initialized with Fallback Anthropic model: {ordered[0]}")
            return bound
        except Exception as e:
...
class EidikoAgentWorkflow:
    def __init__(self, aggregator: SseAggregator = None, model_name: str = None):
        ...
        req_name = self.model_name.lower()
    ...
    async def _call_agent(self, state: AgentState) -> Dict[str, Any]:
        ...
        if self.llm:
            groq_candidates = list(dict.fromkeys([LLM_MODEL] + GROQ_MODEL_CANDIDATES))
            anthropic_candidates = list(dict.fromkeys([FALLBACK_LLM_MODEL] + ANTHROPIC_MODEL_CANDIDATES))

            for attempt in range(len(groq_candidates) + len(anthropic_candidates) + 1):
                try:
                    ...
                    if self._using_groq:
                        ...
                        if self._current_model_idx < len(groq_candidates):
                            ...
                            print(f"[LLM] ⚠️ Main Groq model error. Trying next candidate: {next_model}")
                            try:
                        ...
                        if ANTHROPIC_API_KEY and ANTHROPIC_API_KEY not in ("", "your_anthropic_api_key_here"):
                            ...
                            self._current_model_idx = 0
                            next_model = anthropic_candidates[0]
                            print(f"[LLM] ⚠️ Main Groq model failed. Switching to Fallback Anthropic model: {next_model}")
                            try:
                        ...
                        if self._current_model_idx < len(anthropic_candidates):
                            ...
                            print(f"[LLM] ⚠️ Fallback Claude model error. Trying: {next_model}")
                            try:
                                self.llm = _build_anthropic_llm(next_model, self.tools)
                                continue
                            except Exception:
                    ...
                    self.llm = None
                    return {"messages": [AIMessage(content=f"⚠️ **Model Error**: Unable to reach model `{self.model_name}`. Details: {err}")]}
