...
class RemediationRequest(Base):
    """A human decision and execution audit for one controlled remediation."""
    ...
    failure_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("failures.id", ondelete="SET NULL"),
    ...
    resource_name: Mapped[str] = mapped_column(String(253), nullable=False, index=True)
    ...
    execution_mode: Mapped[str] = mapped_column(
        String(16), nullable=False, default="DRY_RUN"
    )
    result_message: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    pre_state: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
